...
//		widgets.add(Widgets.createSlot(new Point(bgBounds.x + 85, bgBounds.y + 18)).entries(display.getOutputEntries().get(0)).markOutput().disableBackground());
...
//		Arrow cookArrow = Widgets.createArrow(new Point(bgBounds.x + 61, bgBounds.y + 10)).animationDurationTicks(display.getBrewTime());
//		widgets.add(cookArrow);
//		widgets.add(Widgets.createLabel(new Point(cookArrow.getBounds().x + cookArrow.getBounds().width / 2, cookArrow.getBounds().y - 8), new TextComponent(display.getBrewTime() + " t")).noShadow().centered().tooltipLine("Ticks").color(ChatFormatting.DARK_GRAY.getColor(), ChatFormatting.GRAY.getColor()));
//		return widgets;
//	}
//}
