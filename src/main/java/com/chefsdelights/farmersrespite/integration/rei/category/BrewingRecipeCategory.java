...
public class BrewingRecipeCategory implements DisplayCategory<BrewingRecipeDisplay> {
	...
	public List<Widget> setupDisplay(BrewingRecipeDisplay display, Rectangle bounds) {
		...
		if(display.getNeedWater() == true){
			...
			widgets.add(Widgets.createLabel(new Point(bgBounds.x + 5, bgBounds.y + 23), new TextComponent(" ")).noShadow().leftAligned().tooltip(FarmersRespite.i18n("rei.brewing.needWater")));
		}
		else{
			widgets.add(Widgets.createLabel(new Point(bgBounds.x + 5, bgBounds.y + 23), new TextComponent(" ")).noShadow().tooltip(FarmersRespite.i18n("rei.brewing.noWater")));
		}
		...
		widgets.add(Widgets.createLabel(new Point(cookArrow.getBounds().x + cookArrow.getBounds().width / 2, cookArrow.getBounds().y - 10), new TextComponent(display.getBrewTime() + " t")).noShadow().centered().tooltip(new TextComponent("Ticks")).color(ChatFormatting.DARK_GRAY.getColor(), ChatFormatting.GRAY.getColor()));
		return widgets;
	}
}
